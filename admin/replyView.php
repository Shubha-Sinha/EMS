...
<body>
    ...
    <section class="container my-5">
      ...
      ';
        ...
        $task_id = $_GET['id'];
        // fetch start ********************
        $sql1 = "SELECT * FROM `reply` JOIN `user` on `user`. `id`  = `reply` . `reply_by`  WHERE taskId =  " . $task_id . "";
        $result1 = mysqli_query($conn, $sql1);
        if (mysqli_num_rows($result1) > 0) {
            echo ' <p class="mt-2" style="text-align: right;">Task result</p>';

            while ($row1 = mysqli_fetch_assoc($result1)) {
                echo '
      <div class="my-2" style="background-color: #abcdef; border: 2px black; border-radius: 20px; padding: 18px;">
      <strong>' . $row1['name'] . '</strong> <i class="bi bi-caret-right-fill"></i> ' . $row1['reply'] . '
      </div>
      ';
            }
        }  ?>
        ...
        <form action="" method="post">
            ...
            <div class="col-md-2 mx-auto mt-1">
                ...
                <a href="givenTask.php" class="btn btn-primary">Cancel</a>
            </div>
        </form>
    </section>
